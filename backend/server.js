...
// 4. GET STUDENTS BY SECTION - TEMPORARILY WITHOUT AUTH FOR TESTING
app.get('/api/admin/students', async (req, res) => {
    try {
        const { section } = req.query;
        
        console.log('📥 Request to /api/admin/students with section:', section);
        ...
        const result = await pool.query(
            `SELECT uid, name, email, profile_pic, face_ready, section, created_at
             FROM students 
             WHERE section = $1 
             ORDER BY name`,
            [section]
...
app.listen(PORT, () => {
    console.log(`
╔══════════════════════════════════════════════════════════════════════════╗
║                    CUIMS ATTENDANCE SYSTEM - BACKEND                    ║
║                         Server Active - ENHANCED                        ║
╚══════════════════════════════════════════════════════════════════════════╝
...
🚀 TEST ENDPOINTS:
   • GET    /api/cors-test              - Test CORS connection
...
🔍 To test connection, visit: http://localhost:${PORT}/api/test-connection
    `);
});
